...
class WipSeleccionada(BaseModel):
    ...
    factor_ajuste: float = Field(default=1.0, ge=0.1, le=2.0, description="Factor de ajuste")
...
class CotizacionInput(BaseModel):
    ...
    cliente_marca: str = Field(..., min_length=1, max_length=100, description="Cliente/Marca")
    ...
    familia_producto: str = Field(..., min_length=1, max_length=100, description="Familia de producto")
    tipo_prenda: str = Field(..., min_length=1, max_length=100, description="Tipo de prenda")
    codigo_estilo: str = Field(..., min_length=1, max_length=50, description="Código del estilo")
    usuario: str = Field(default="Sistema", max_length=100, description="Usuario que cotiza")
...
class AutoCompletadoInfo(BaseModel):
    ...
    familia_producto: Optional[str] = Field(None, description="Familia de producto sugerida")
...
class InfoEstiloDetallada(BaseModel):
    ...
    ultima_facturacion: Optional[datetime] = Field(None, description="Última facturación")
    primera_facturacion: Optional[datetime] = Field(None, description="Primera facturación")
    categoria: TipoEstilo = Field(default=TipoEstilo.NUEVO, description="Categoría del estilo")
    ...
    version_calculo: VersionCalculo = Field(default=VersionCalculo.FLUIDA, description="Versión usada")
...
class VerificacionEstiloCompleta(BaseModel):
    ...
    autocompletado: AutoCompletadoInfo = Field(..., description="Info de auto-completado")
    info_detallada: Optional[InfoEstiloDetallada] = Field(None, description="Información detallada si existe")
...
class CostosComponentesOP(BaseModel):
    ...
    materia_prima: float = Field(default=0.0, description="Costo materia prima unitario")
    indirecto_fijo: float = Field(default=0.0, description="Costo indirecto fijo unitario")
...
class OpReal(BaseModel):
    ...
    fecha_facturacion: Optional[datetime] = Field(None, description="Fecha de facturación")
    ...
    total_ajustes_aplicados: Optional[int] = Field(default=0, description="Número de ajustes aplicados")
...
class EstadisticasOPs(BaseModel):
    ...
    esfuerzo_promedio: float = Field(default=6.0, ge=1, le=10, description="Esfuerzo promedio")
    ops_con_ajustes: Optional[int] = Field(default=0, description="OPs con ajustes aplicados")
    rango_fechas: Optional[Dict[str, str]] = Field(None, description="Rango de fechas de las OPs")
...
class OpsDataResponse(BaseModel):
    ...
    metodo_utilizado: str = Field(..., description="Método usado para encontrar las OPs")
    estadisticas: EstadisticasOPs = Field(..., description="Estadísticas de las OPs")
    parametros_busqueda: ParametrosBusquedaOPs = Field(..., description="Parámetros de búsqueda")
    limites_aplicados: Optional[bool] = Field(default=False, description="Si se aplicaron límites")
...
class OpsUtilizadasResponse(BaseModel):
    ...
    timestamp: datetime = Field(default_factory=datetime.now, description="Momento de la consulta")
    total_ops_encontradas: int = Field(..., ge=0, description="Total de OPs encontradas")
    parametros_entrada: Dict[str, Any] = Field(..., description="Parámetros de entrada originales")
...
class InfoComercial(BaseModel):
    ...
    tendencias_costos: List[Dict[str, Any]] = Field(..., description="Tendencias de costos")
    analisis_competitividad: Optional[List[Dict[str, Any]]] = Field(None, description="Análisis competitivo")
...
class WipRecomendada(BaseModel):
    ...
    recomendacion: str = Field(..., description="Nivel de recomendación (Alta/Media/Baja)")

class RutaTextilResponse(BaseModel):
    """✅ NUEVO: Respuesta de ruta textil recomendada"""
    tipo_prenda: str = Field(..., description="Tipo de prenda")
    version_calculo: VersionCalculo = Field(..., description="Versión de cálculo usada")
    wips_recomendadas: List[WipRecomendada] = Field(..., description="WIPs recomendadas")
    wips_textiles_recomendadas: List[WipRecomendada] = Field(..., description="WIPs textiles específicas")
    wips_manufactura_recomendadas: List[WipRecomendada] = Field(..., description="WIPs manufactura específicas")
    ...
    fecha_analisis: datetime = Field(default_factory=datetime.now, description="Fecha del análisis")
...
class CotizacionResponse(BaseModel):
    ...
    costo_indirecto_fijo: float = Field(..., description="Costo indirecto fijo unitario")
    ...
    volumen_historico: Optional[int] = Field(None, description="Volumen histórico del estilo")
    estrategia_costos: Optional[str] = Field(None, description="Estrategia de costos usada")
    configuracion_wips: Optional[List[WipConfiguracion]] = Field(default_factory=list, description="WIPs configuradas")
    metadatos_adicionales: Optional[Dict[str, Any]] = Field(default_factory=dict, description="Metadatos adicionales")
    timestamp: Optional[datetime] = Field(None, description="Timestamp de la cotización")
    usuario: Optional[str] = Field(None, description="Usuario que realizó la cotización")
...
class WipsDisponiblesResponse(BaseModel):
    ...
    tipo_prenda_filtro: Optional[str] = Field(None, description="Tipo de prenda filtrado")
    version_calculo: VersionCalculo = Field(default=VersionCalculo.FLUIDA, description="Versión de cálculo usada")
