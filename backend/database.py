...
from config import settings, db_config, factores
from models import EstiloSimilar, WipDisponible
...
class TDVQueries:
    ...
    def obtener_fecha_maxima_corrida(self, tabla: str, version_calculo: str = 'FLUIDA') -> datetime:
        """Obtiene la fecha_corrida máxima de cada tabla para una versión específica"""
        
        # ✅ CORRECCIÓN: HISTORIAL_ESTILOS no tiene version_calculo
        if tabla == 'HISTORIAL_ESTILOS':
            query = "SELECT MAX(fecha_corrida) as fecha_max FROM TDV.saya.HISTORIAL_ESTILOS"
    ...
    def verificar_estilo_existente(self, codigo_estilo: str, version_calculo: str = 'FLUIDA') -> bool:
        """
        ...
        try:
            ...
            resultado_historial = self.db.execute_query(query_historial, (codigo_estilo,))
            existe_en_historial = (resultado_historial[0]['total_historial'] > 0) if resultado_historial else False
            ...
            resultado_ops = self.db.execute_query(query_ops, (codigo_estilo, version_calculo, version_calculo, version_calculo))
            ...
            # ✅ LOGGING DETALLADO
            logger.info(f"🔍 Verificación estilo '{codigo_estilo}' ({version_calculo}): "
    ...
    def obtener_info_detallada_estilo(self, codigo_estilo: str, version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        """
        ...
        try:
            ...
            resultado = self.db.execute_query(query, (codigo_estilo, version_calculo, version_calculo, version_calculo))
            ...
            if resultado:
                ...
                return {
                    ...
                    'esfuerzo_promedio': float(info['esfuerzo_promedio']) if info['esfuerzo_promedio'] else 6,
            ...
            logger.info(f"🔍 Estilo {codigo_estilo} no en historial, buscando en OPs directamente...")
            ...
            resultado_fallback = self.db.execute_query(query_fallback, (codigo_estilo, version_calculo, version_calculo, version_calculo))
            ...
            if resultado_fallback:
                ...
                if total_ops >= 2:
                    ...
                    logger.info(f"🔍 Estilo {codigo_estilo} tiene solo {total_ops} OP(s), considerado NUEVO")
            ...
            logger.error(f"❌ Error obteniendo info detallada estilo {codigo_estilo}: {e}")
            return {
                'encontrado': False, 
                'error': str(e),
                'codigo_estilo': codigo_estilo,
    ...
    def buscar_estilos_similares(self, codigo_estilo: str, cliente: str, limite: int = 10, 
                                version_calculo: str = 'FLUIDA') -> List[EstiloSimilar]:
        ...
        params = (f'{prefijo}%', f'%{cliente}%', version_calculo, version_calculo, version_calculo)
        resultados = self.db.execute_query(query, params)
        
        estilos = []
        for row in resultados[:limite]:
            estilos.append(EstiloSimilar(
                codigo=row['codigo'],
        ...
        logger.info(f"✅ Estilos similares encontrados: {len(estilos)} para {codigo_estilo} ({version_calculo})")
        return estilos
    ...
    def obtener_clientes_disponibles(self, version_calculo: str = 'FLUIDA') -> List[str]:
        ...
        resultados = self.db.execute_query(query, (version_calculo, version_calculo, version_calculo))
        clientes = [row['cliente'] for row in resultados]
        logger.info(f"✅ Clientes cargados para {version_calculo}: {len(clientes)}")
        return clientes
    
    def obtener_familias_productos(self, version_calculo: str = 'FLUIDA') -> List[str]:
    ...
    def obtener_tipos_prenda(self, familia: str, version_calculo: str = 'FLUIDA') -> List[str]:
        ...
        resultados = self.db.execute_query(query, (familia, version_calculo, version_calculo, version_calculo))
        tipos = [row['tipo_de_producto'] for row in resultados]
        logger.info(f"✅ Tipos cargados para {familia} ({version_calculo}): {len(tipos)}")
        return tipos
    ...
    def buscar_costos_historicos(self, familia_producto: str, tipo_prenda: str, 
                                cliente: Optional[str] = None, version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        ...
        if cliente:
            ...
            resultados = self.db.execute_query(query_exacta, 
                (familia_producto, tipo_prenda, cliente, version_calculo, version_calculo, version_calculo))
            
            if resultados:
                logger.info(f"✅ Costos encontrados con cliente específico: {len(resultados)} registros")
                return self._procesar_costos_historicos_con_limites_previos(resultados, "exacta_con_cliente")
        ...
        resultados = self.db.execute_query(query_familia, 
            (familia_producto, tipo_prenda, version_calculo, version_calculo, version_calculo))
        
        if resultados:
            logger.info(f"✅ Costos encontrados por familia/tipo: {len(resultados)} registros")
            return self._procesar_costos_historicos_con_limites_previos(resultados, "familia_tipo")
        
        raise ValueError(f"No se encontraron costos históricos para {familia_producto} - {tipo_prenda}")
    
    def buscar_costos_estilo_especifico(self, codigo_estilo: str, meses: int = 12, 
                                       version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        """
        ...
        resultados = self.db.execute_query(query, (codigo_estilo, version_calculo, version_calculo, meses, version_calculo))
        
        if not resultados:
            logger.warning(f"⚠️ No se encontraron costos históricos para estilo {codigo_estilo}")
            raise ValueError(f"No se encontraron costos históricos para el estilo específico: {codigo_estilo}")
        
        logger.info(f"✅ Costos estilo específico encontrados: {len(resultados)} registros para {codigo_estilo}")
        return self._procesar_costos_historicos_con_limites_previos(resultados, "estilo_especifico")
    
    def _procesar_costos_historicos_con_limites_previos(self, resultados: List[Dict], estrategia: str) -> Dict[str, Any]:
        ...
        for columna in ['costo_textil', 'costo_manufactura', 'costo_avios', 'costo_materia_prima',
                    'costo_indirecto_fijo', 'gasto_administracion', 'gasto_ventas']:
            
            df[columna] = pd.to_numeric(df[columna], errors='coerce').fillna(0).astype(float)
            df['prendas_requeridas'] = pd.to_numeric(df['prendas_requeridas'], errors='coerce').fillna(1).astype(float)
            df[f'{columna}_unitario'] = df[columna] / df['prendas_requeridas'].clip(lower=1.0)
            ...
            if columna in factores.RANGOS_SEGURIDAD:
                ...
                df[f'{columna}_unitario'] = df[f'{columna}_unitario'].clip(lower=rango['min'], upper=rango['max'])
            ...
            promedio_ponderado = (df[f'{columna}_unitario'] * df['peso_temporal']).sum() / suma_pesos
            resultado[columna] = float(promedio_ponderado)
        
        resultado.update({
            'esfuerzo_promedio': float(df['esfuerzo_total'].mean() if 'esfuerzo_total' in df.columns else 6),
        ...
        logger.info(f"✅ Costos procesados: {len(resultados)} registros, {sum(registros_ajustados.values())} ajustes aplicados")
    ...
    def obtener_costos_wips_por_estabilidad(self, tipo_prenda: str, version_calculo: str = 'FLUIDA') -> Dict[str, float]:
        """Usar análisis inteligente de variación"""
        try:
            return self.obtener_costos_wips_inteligente(tipo_prenda, version_calculo)
        except Exception as e:
            logger.warning(f"⚠️ Error en análisis inteligente, usando método legacy: {e}")
            return self._obtener_costos_wips_legacy(tipo_prenda, version_calculo)

    def obtener_costos_wips_inteligente(self, tipo_prenda: str, version_calculo: str = 'FLUIDA') -> Dict[str, float]:
        """
        ...
        resultados_variabilidad = self.db.execute_query(query_variabilidad, 
            (tipo_prenda, version_calculo, version_calculo, version_calculo, tipo_prenda))
        ...
        logger.info(f"🔍 Análisis variabilidad: {len(wips_data)} WIPs encontrados para {tipo_prenda} ({version_calculo})")
        ...
        for wip_id, costos_mensuales in wips_data.items():
            if len(costos_mensuales) < 2:
                ...
                costos_wips[wip_id] = float(sum(costos_mensuales) / len(costos_mensuales))
                logger.debug(f"WIP {wip_id}: POCOS DATOS ({len(costos_mensuales)} meses) - Promedio simple: ${costos_wips[wip_id]:.2f}")
                continue
            ...
            try:
                ...
                desviacion = statistics.stdev(costos_mensuales) if len(costos_mensuales) > 1 else 0
            ...
            if coef_variacion <= 0.10:
                periodo_meses = 3
                logger.debug(f"WIP {wip_id}: ESTABLE (CV: {coef_variacion:.1%}) - Usando 3 meses")
            else:
                periodo_meses = 6
                logger.debug(f"WIP {wip_id}: VARIABLE (CV: {coef_variacion:.1%}) - Usando 6 meses")
            ...
            if wip_id in ['37', '45']:
                ...
                AND costo_por_prenda > 0
                """
                resultado_wip = self.db.execute_query(query_wip, 
                    (wip_id, tipo_prenda, version_calculo, version_calculo, periodo_meses, version_calculo, tipo_prenda))
            else:
                if coef_variacion <= 0.10:
                    ...
                    ORDER BY mes DESC
                    """
                    resultado_wip = self.db.execute_query(query_wip, (wip_id, tipo_prenda, version_calculo, version_calculo))
                else:
        ...
        logger.info(f"✅ Análisis inteligente completado: {len(costos_wips)} WIPs para {tipo_prenda} ({version_calculo})")
        return costos_wips

    def obtener_ruta_textil_recomendada(self, tipo_prenda: str, version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        """
        ✅ FUNCIÓN SIN CAMBIOS: Ya usa fechas relativas correctamente
        """
        ...
        resultados = self.db.execute_query(query_ruta, (tipo_prenda, version_calculo, version_calculo, version_calculo, tipo_prenda))
        ...
        for row in resultados:
            wip_data = {
                'wip_id': row['wip_id'],
                'nombre': factores.NOMBRES_WIPS.get(row['wip_id'], f"WIP {row['wip_id']}"),
                'frecuencia_uso': int(row['frecuencia_uso']),
                'costo_promedio': float(row['costo_promedio']),
                'ultimo_uso': row['ultimo_uso'],
                'prendas_promedio': float(row['prendas_promedio']) if row['prendas_promedio'] else 0,
                'total_prendas_acumulado': int(row['total_prendas_acumulado']) if row['total_prendas_acumulado'] else 0,
                'recomendacion': 'Alta' if row['frecuencia_uso'] >= 6 else 'Media' if row['frecuencia_uso'] >= 3 else 'Baja'
            }
        ...
        logger.info(f"✅ Ruta textil obtenida para {tipo_prenda} ({version_calculo}): {len(wips_recomendadas)} WIPs")
    ...
    def _obtener_costos_wips_legacy(self, tipo_prenda: str, version_calculo: str = 'FLUIDA') -> Dict[str, float]:
        ...
        resultados_inestables = self.db.execute_query(query_inestables, 
            (tipo_prenda, version_calculo, version_calculo, version_calculo, tipo_prenda))
        ...
        resultados_estables = self.db.execute_query(query_estables, (tipo_prenda, version_calculo, version_calculo))
        for row in resultados_estables:
            costos_wips[row['wip_id']] = float(row['costo_por_prenda'])
        
        logger.info(f"✅ Método legacy: {len(costos_wips)} WIPs encontrados para {tipo_prenda} ({version_calculo})")
        return costos_wips

    def obtener_wips_disponibles_estructurado(self, tipo_prenda: Optional[str] = None, 
                                             version_calculo: str = 'FLUIDA') -> Tuple[List[WipDisponible], List[WipDisponible]]:
        """✅ SIN CAMBIOS: Ya usa lógica correcta"""
        
        try:
            if tipo_prenda:
                logger.info(f"🔍 Obteniendo WIPs específicos para {tipo_prenda} ({version_calculo})")
                costos_wips = self.obtener_costos_wips_por_estabilidad(tipo_prenda, version_calculo)
            else:
                ...
                resultados = self.db.execute_query(query, (version_calculo, version_calculo))
                costos_wips = {}
                for row in resultados:
                    costos_wips[row['wip_id']] = float(row['costo_promedio'])
            ...
            for wip_id in factores.WIPS_TEXTILES:
                ...
                wips_textiles.append(WipDisponible(
                    wip_id=wip_id,
                    nombre=factores.NOMBRES_WIPS.get(wip_id, f"WIP {wip_id} - Textil"),
            ...
            for wip_id in factores.WIPS_MANUFACTURA:
                costo_actual = costos_wips.get(wip_id, 0.0)
                wips_manufactura.append(WipDisponible(
                    wip_id=wip_id,
                    nombre=factores.NOMBRES_WIPS.get(wip_id, f"WIP {wip_id} - Manufactura"),
                    costo_actual=costo_actual,
                    disponible=costo_actual > 0,
                    grupo="Manufactura"
                ))
            
            logger.info(f"✅ WIPs estructurados: {len(wips_textiles)} textiles, {len(wips_manufactura)} manufactura")
    ...
    # ✅ CÓDIGO CORREGIDO
    def obtener_gastos_indirectos_formula(self, version_calculo: str = 'FLUIDA') -> Dict[str, float]:
        ...
        resultado = self.db.execute_query(query, (version_calculo, version_calculo, version_calculo))
        if resultado:
            gastos = {
            ...
            logger.info(f"✅ Gastos indirectos UNITARIOS obtenidos ({version_calculo}): {gastos}")
            return gastos
    ...
    def obtener_ultimo_costo_materiales(self, version_calculo: str = 'FLUIDA') -> Dict[str, float]:
        ...
        if resultado:
            ...
            prendas = float(registro['prendas_requeridas']) if registro['prendas_requeridas'] else 1.0
            
            costo_materia_prima_unitario = float(registro['costo_materia_prima']) / prendas
            costo_avios_unitario = float(registro['costo_avios']) / prendas
            
            logger.info(f"✅ Último costo materiales ({version_calculo}): OP {registro['cod_ordpro']} - "
        ...
        logger.warning(f"⚠️ No se encontró último costo de materiales válido para {version_calculo}")
        return {'costo_materia_prima': 0, 'costo_avios': 0}
    
    def obtener_volumen_historico_estilo(self, codigo_estilo: str, version_calculo: str = 'FLUIDA') -> int:
        """
        ✅ SIN CAMBIOS: Ya usa lógica correcta con fecha_corrida
        """
        ...
        resultado = self.db.execute_query(query, (codigo_estilo, version_calculo, version_calculo))
        volumen = int(resultado[0]['volumen_total']) if resultado and resultado[0]['volumen_total'] else 0
        
        logger.info(f"✅ Volumen histórico {codigo_estilo} ({version_calculo}): {volumen} prendas")
        return volumen
    ...
    def obtener_ops_utilizadas_cotizacion(self, codigo_estilo: str = None, 
                                 familia_producto: str = None,
                                 tipo_prenda: str = None, 
                                 cliente: str = None,
                                 version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        """
        ...
        if codigo_estilo:
            try:
                ...
                resultados = self.db.execute_query(query_estilo, 
        ...
        if not ops_utilizadas and familia_producto and tipo_prenda:
            try:
                params = [familia_producto, tipo_prenda, version_calculo, version_calculo, version_calculo]
                cliente_filter = ""
        ...
        for op in ops_utilizadas:
            ...
            costo_manufactura_ajustado, manufactura_ajustado = factores.validar_rango_seguridad(
            ...
            costo_indirecto_ajustado, indirecto_ajustado = factores.validar_rango_seguridad(
                float(op.get('costo_indirecto_unit', 0) or 0), 'costo_indirecto_fijo'
            )
            ...
            costo_total_ajustado = (
                costo_textil_ajustado + costo_manufactura_ajustado + 
                costo_avios_ajustado + costo_mp_ajustado + 
                costo_indirecto_ajustado + gasto_admin_ajustado + 
                gasto_ventas_ajustado
            )
            
            # ✅ PASO 4: Determinar si fue ajustado
            fue_ajustado = (textil_ajustado or manufactura_ajustado or avios_ajustado or 
                        mp_ajustado or indirecto_ajustado or admin_ajustado or ventas_ajustado)
            
            ops_procesadas.append({
                'cod_ordpro': op['cod_ordpro'],
                'estilo_propio': op.get('estilo_propio', codigo_estilo),
                'cliente': op['cliente'],
                'fecha_facturacion': op['fecha_facturacion'].isoformat() if op['fecha_facturacion'] else None,
                'prendas_requeridas': int(op['prendas_requeridas']),
                'monto_factura': float(op['monto_factura']),
                'precio_unitario': float(op['monto_factura'] / op['prendas_requeridas']) if op['prendas_requeridas'] > 0 else 0,
        ...
        if ops_procesadas:
            costos_totales_ajustados = [op['costo_total_unitario'] for op in ops_procesadas]
            costos_totales_originales = [op['costo_total_original'] for op in ops_procesadas]
            esfuerzos = [op['esfuerzo_total'] for op in ops_procesadas]
            
            estadisticas = {
                'total_ops': len(ops_procesadas),
                'costo_promedio': sum(costos_totales_ajustados) / len(costos_totales_ajustados),
                'costo_min': min(costos_totales_ajustados),
                'costo_max': max(costos_totales_ajustados),
                'costo_promedio_original': sum(costos_totales_originales) / len(costos_totales_originales),
                ...
                'ops_con_ajustes': sum(1 for op in ops_procesadas if op['fue_ajustado']),
                'rango_fechas': {
                    'desde': min([op['fecha_facturacion'] for op in ops_procesadas if op['fecha_facturacion']]),
                    'hasta': max([op['fecha_facturacion'] for op in ops_procesadas if op['fecha_facturacion']])
                }
            }
        else:
            estadisticas = {
        ...
        logger.info(f"✅ OPs utilizadas procesadas: {len(ops_procesadas)} encontradas con método {metodo_usado}")
    ...
    def obtener_info_comercial(self, familia_producto: str, tipo_prenda: str, 
                              version_calculo: str = 'FLUIDA') -> Dict[str, Any]:
        ...
        resultado_volumen = self.db.execute_query(query_volumen, 
        ...
        tendencias = self.db.execute_query(query_tendencias, 
            (familia_producto, tipo_prenda, version_calculo, version_calculo))
        ...
        competitivo = self.db.execute_query(query_competitivo, 
            (familia_producto, tipo_prenda, version_calculo, version_calculo))
        ...
        info_comercial = {
            'historico_volumen': {
                ...
                'volumen_promedio': float(round(volumen.get('volumen_promedio', 0) or 0, 1))
            },
            'tendencias_costos': [
                {
                    'año': int(t['año']),
                    ...
                    'costo_textil_promedio': float(round(t['costo_textil_promedio'] or 0, 4)),
                    'costo_manufactura_promedio': float(round(t['costo_manufactura_promedio'] or 0, 4))
                }
                for t in tendencias
            ],
            'analisis_competitividad': [
                {
                    ...
                    'precio_promedio_cliente': float(round(c['precio_promedio_cliente'] or 0, 2))
                }
        ...
        logger.info(f"✅ Info comercial obtenida para {familia_producto}/{tipo_prenda} ({version_calculo})")
