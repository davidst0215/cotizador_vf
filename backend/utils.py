...
from models import (
    CotizacionInput, CotizacionResponse, ComponenteCosto, 
    WipConfiguracion, InfoComercial, WipSeleccionada
)
...
class CotizadorTDV:
    ...
    def procesar_cotizacion(self, input_data: CotizacionInput) -> CotizacionResponse:
        ...
        logger.info(f"💰 Iniciando cotización: {input_data.codigo_estilo} | {input_data.usuario} | Versión: {input_data.version_calculo}")
        ...
        try:
            ...
            categoria_estilo, volumen_historico, info_autocompletado = self._determinar_categoria_estilo_completa(input_data)
            logger.info(f"🔍 Estilo {input_data.codigo_estilo}: categoría={categoria_estilo}, volumen={volumen_historico}")
            
            # ✅ AUTO-COMPLETADO AUTOMÁTICO
            if info_autocompletado and info_autocompletado.get('encontrado'):
                logger.info(f"🎯 Aplicando auto-completado para {input_data.codigo_estilo}")
                input_data.familia_producto = info_autocompletado.get('familia_producto', input_data.familia_producto)
                input_data.tipo_prenda = info_autocompletado.get('tipo_prenda', input_data.tipo_prenda)
            ...
            if es_estilo_nuevo:
                logger.info(f"🆕 Procesando como ESTILO NUEVO: {input_data.codigo_estilo}")
                resultado = self._procesar_estilo_nuevo_mejorado(
                    input_data, id_cotizacion, categoria_lote, factor_lote, factor_marca
                )
            else:
                logger.info(f"🔄 Procesando como ESTILO RECURRENTE: {input_data.codigo_estilo}")
            ...
            if es_estilo_nuevo and input_data.familia_producto and input_data.tipo_prenda:
                ...
                if ruta_automatica:
                    ...
                    resultado.metadatos_adicionales["ruta_automatica_sugerida"] = ruta_automatica
                    resultado.metadatos_adicionales["estilo_nuevo_con_ruta"] = True
                    logger.info(f"🧵 Ruta automática agregada para estilo nuevo: {len(ruta_automatica.get('wips_recomendadas', []))} WIPs")
            ...
            logger.info(f"✅ Cotización completada: {id_cotizacion} | ${resultado.precio_final:.2f} | {tiempo_procesamiento:.2f}s")
    ...
    def _validar_input_completo(self, input_data: CotizacionInput):
        ...
        if not input_data.familia_producto or len(input_data.familia_producto.strip()) == 0:
        ...
        if not hasattr(input_data, 'cantidad_prendas') or not input_data.cantidad_prendas:
            cantidad_map = {
                'Micro Lote': 25,
                'Lote Pequeño': 250,
                'Lote Mediano': 750,
            ...
            input_data.cantidad_prendas = cantidad_map.get(input_data.categoria_lote, 500)
    ...
    def _determinar_categoria_estilo_completa(self, input_data: CotizacionInput) -> Tuple[str, int, Optional[Dict]]:
        """
        ...
        if input_data.codigo_estilo:
            try:
                ...
                if info_detallada.get('encontrado', False):
                    ...
                    logger.info(f"🔍 Estilo {input_data.codigo_estilo} ENCONTRADO: volumen={volumen_total}, "
                              f"categoría={categoria_estilo}, fuente={info_detallada.get('fuente')}")
                else:
                    # No encontrado, es nuevo
                    categoria_estilo = "Nuevo"
                    logger.info(f"🆕 Estilo {input_data.codigo_estilo} NO ENCONTRADO: categoría=Nuevo")
                    
            except Exception as e:
                logger.error(f"❌ Error determinando categoría estilo {input_data.codigo_estilo}: {e}")
                categoria_estilo = "Nuevo"  # Asumir nuevo en caso de error
        
        return categoria_estilo, volumen_historico, info_autocompletado
    
    def _obtener_ruta_automatica_mejorada(self, input_data: CotizacionInput) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            if not ruta_textil or not ruta_textil.get('wips_recomendadas'):
                logger.warning(f"⚠️ No se encontró ruta textil para {input_data.tipo_prenda}")
                return None
            
            # Obtener WIPs disponibles estructurados
            wips_textiles, wips_manufactura = tdv_queries.obtener_wips_disponibles_estructurado(
                input_data.tipo_prenda, 
                input_data.version_calculo
            )
            ...
            wips_textiles_disponibles = [
                ...
                for w in wips_textiles if w.disponible
            ][:5]  # Top 5
            ...
            wips_manufactura_disponibles = [
                ...
                for w in wips_manufactura if w.disponible
            ][:5]  # Top 5
            ...
            ruta_automatica = {
                ...
                "total_disponibles": len(wips_textiles_disponibles) + len(wips_manufactura_disponibles),
                "mensaje": f"Ruta automática generada para estilo nuevo '{input_data.codigo_estilo}' en {input_data.tipo_prenda}",
                "fecha_generacion": datetime.now().isoformat()
            }
            
            logger.info(f"🧵 Ruta automática generada: {len(wips_textiles_disponibles)} textiles + {len(wips_manufactura_disponibles)} manufactura")
            return ruta_automatica
            
        except Exception as e:
            logger.warning(f"⚠️ Error generando ruta automática para {input_data.codigo_estilo}: {e}")
            return None
    
    def _procesar_estilo_recurrente_mejorado(self, input_data: CotizacionInput, id_cotizacion: str,
                                       categoria_lote: str, factor_lote: float, 
                                       factor_marca: float) -> CotizacionResponse:
        """
        ✅ FUNCIÓN COMPLETAMENTE CORREGIDA: Procesa estilos recurrentes 
        TODOS los costos del histórico específico del estilo
        """
        
        logger.info(f"🔄 PROCESANDO estilo RECURRENTE: {input_data.codigo_estilo} ({input_data.version_calculo})")
        ...
        try:
            ...
            logger.info(f"✅ Costos históricos COMPLETOS obtenidos: {costos_hist.get('registros_encontrados', 0)} registros")
            
        except Exception as e:
            logger.warning(f"⚠️ No se pudo obtener costos completos por estilo específico: {e}")
            # ❌ Si no tiene histórico completo, NO es verdaderamente recurrente
            raise ValueError(f"Estilo {input_data.codigo_estilo} marcado como recurrente pero sin histórico completo")
        ...
        for componente in componentes_esperados:
            ...
            if valor is None or valor <= 0:
                ...
                logger.warning(f"⚠️ {componente} no disponible en histórico de {input_data.codigo_estilo}")
                ...
                if componente in factores.RANGOS_SEGURIDAD:
                    ...
                    alertas.append(f"⚠️ {componente}: no en histórico, usando valor mínimo ${valor:.2f}")
                else:
                    valor = 0.5  # Fallback genérico
                    alertas.append(f"⚠️ {componente}: no en histórico, usando fallback ${valor:.2f}")
            
            # Aplicar rangos de seguridad
            valor_original = valor
            valor_validado, fue_ajustado = factores.validar_rango_seguridad(valor, componente)
            costos_validados[componente] = valor_validado
            
            if fue_ajustado:
                alertas.append(f"⚠️ {componente}: ajustado de ${valor_original:.2f} a ${valor_validado:.2f}")
            
            # Determinar fuente real
            fuente_real = "historico" if valor_original > 0 else "fallback_minimo"
            
            componentes.append(ComponenteCosto(
        ...
        if componentes_faltantes:
            alertas.append(f"⚠️ Componentes sin datos históricos: {', '.join(componentes_faltantes)}")
            logger.warning(f"⚠️ Estilo {input_data.codigo_estilo} recurrente pero faltan: {componentes_faltantes}")
        ...
        volumen = tdv_queries.obtener_volumen_historico_estilo(input_data.codigo_estilo, input_data.version_calculo)
        ...
        validaciones = [
            f"✅ Estilo RECURRENTE procesado con histórico específico",
        ...
        if costos_hist.get('total_ajustados', 0) > 0:
            alertas.append(f"⚠️ {costos_hist.get('total_ajustados')} valores ajustados por límites de seguridad")
        
        if len(componentes_faltantes) > 0:
            alertas.append(f"⚠️ Estilo recurrente incompleto: {len(componentes_faltantes)} componentes con fallback")
        ...
        return CotizacionResponse(
            ...
            precision_estimada=costos_hist.get('precision_estimada', 0.95),  # Mayor precisión al usar histórico específico
            ...
            metadatos_adicionales={
                ...
                "componentes_historicos": len(componentes_esperados) - len(componentes_faltantes),
    ...
    def _procesar_estilo_nuevo_mejorado(self, input_data: CotizacionInput, id_cotizacion: str,
                                      categoria_lote: str, factor_lote: float,
                                      factor_marca: float) -> CotizacionResponse:
        """
        ...
        logger.info(f"🆕 PROCESANDO estilo NUEVO: {input_data.codigo_estilo} ({input_data.version_calculo})")
        ...
        if (not input_data.wips_textiles or len(input_data.wips_textiles) == 0) and \
           (not input_data.wips_manufactura or len(input_data.wips_manufactura) == 0):
            raise ValueError("Para estilos nuevos se requieren WIPs textiles y/o manufactura seleccionadas")
        ...
        try:
            ...
            logger.info(f"💰 Costos WIPs obtenidos: {len(costos_wips)} WIPs disponibles")
        except Exception as e:
            logger.error(f"❌ Error obteniendo costos WIPs: {e}")
            raise ValueError(f"No se pudieron obtener costos de WIPs para {input_data.tipo_prenda}")
        ...
        costo_manufactura_total = self._procesar_wips_mejorado(
            input_data.wips_manufactura or [], costos_wips, "Manufactura", componentes, alertas
        )
        
        # ✅ APLICAR RANGOS DE SEGURIDAD
        costo_textil, textil_ajustado = factores.validar_rango_seguridad(costo_textil_total, 'costo_textil')
        costo_manufactura, manufactura_ajustado = factores.validar_rango_seguridad(costo_manufactura_total, 'costo_manufactura')
        
        if textil_ajustado:
            alertas.append(f"⚠️ Costo textil ajustado: ${costo_textil_total:.2f} → ${costo_textil:.2f}")
        if manufactura_ajustado:
            alertas.append(f"⚠️ Costo manufactura ajustado: ${costo_manufactura_total:.2f} → ${costo_manufactura:.2f}")
        
        # ✅ OBTENER COSTOS COMPLEMENTARIOS
        costos_complementarios = self._obtener_costos_complementarios_mejorados(input_data, componentes, alertas)
        
        # ✅ CALCULAR TOTALES
        costo_base_total = costo_textil + costo_manufactura + sum(costos_complementarios.values())
        ...
        validaciones = [
            f"✅ Estilo NUEVO procesado correctamente",
            ...
            f"✅ Estrategia: configuración manual de WIPs"
        ]
        
        # Configuración WIPs para respuesta
        configuracion_wips = []
        for wip in (input_data.wips_textiles or []) + (input_data.wips_manufactura or []):
            costo_base = costos_wips.get(wip.wip_id, 0)
            configuracion_wips.append(WipConfiguracion(
                wip_id=wip.wip_id,
                nombre=factores.NOMBRES_WIPS.get(wip.wip_id, f"WIP {wip.wip_id}"),
                costo=costo_base * wip.factor_ajuste,
                grupo="Textil" if wip.wip_id in factores.WIPS_TEXTILES else "Manufactura"
            ))
    ...
    def _procesar_wips_mejorado(self, wips_seleccionadas: List[WipSeleccionada], 
                               costos_wips: Dict[str, float], grupo: str, 
                               componentes: List[ComponenteCosto], alertas: List[str]) -> float:
        ...
        for wip in wips_seleccionadas:
            ...
            if factor_ajuste < 0.1 or factor_ajuste > 2.0:
                ...
                alertas.append(f"⚠️ Factor ajuste WIP {wip.wip_id} limitado a {factor_ajuste}")
        ...
        logger.info(f"🔧 WIPs {grupo} procesadas: {len(wips_seleccionadas)} → ${costo_total:.2f}")
        return costo_total
    
    def _obtener_costos_complementarios_mejorados(self, input_data: CotizacionInput, 
                                                 componentes: List[ComponenteCosto], 
                                                 alertas: List[str]) -> Dict[str, float]:
        ...
        try:
            costos_materiales = tdv_queries.obtener_ultimo_costo_materiales(input_data.version_calculo)
            for comp in ['costo_materia_prima', 'costo_avios']:
                valor = costos_materiales.get(comp, 0)
                valor_validado, fue_ajustado = factores.validar_rango_seguridad(valor, comp)
            ...
            logger.info(f"💰 Costos materiales obtenidos: MP=${costos_validados.get('costo_materia_prima', 0):.2f}, Avíos=${costos_validados.get('costo_avios', 0):.2f}")
            
        except Exception as e:
            logger.warning(f"⚠️ Error obteniendo costos materiales, usando defaults: {e}")
            for comp in ['costo_materia_prima', 'costo_avios']:
        ...
        try:
            gastos = tdv_queries.obtener_gastos_indirectos_formula(input_data.version_calculo)
            for comp in ['costo_indirecto_fijo', 'gasto_administracion', 'gasto_ventas']:
            ...
            logger.info(f"📊 Gastos indirectos obtenidos: {sum([gastos.get(c, 0) for c in ['costo_indirecto_fijo', 'gasto_administracion', 'gasto_ventas']]):.2f}")
            
        except Exception as e:
            logger.warning(f"⚠️ Error obteniendo gastos indirectos, usando defaults: {e}")
            for comp in ['costo_indirecto_fijo', 'gasto_administracion', 'gasto_ventas']:
                if comp in factores.RANGOS_SEGURIDAD:
                    rango = factores.RANGOS_SEGURIDAD[comp]
                    valor_default = (rango['min'] + rango['max']) / 2
    ...
    def _obtener_info_comercial_mejorada(self, input_data: CotizacionInput) -> InfoComercial:
        ...
        try:
            ...
            return InfoComercial(
                ops_utilizadas=info_raw.get('historico_volumen', {}).get('ops_producidas', 0),
                historico_volumen=info_raw.get('historico_volumen', {
                    "volumen_total_6m": 0,
                    "volumen_promedio": 0,
                    "ops_producidas": 0
                }),
