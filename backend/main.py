...
if sys.platform.startswith('win'):
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('logs/tdv_cotizador.log', encoding='utf-8'),
    ...
    logging.basicConfig(
        ...
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler('logs/tdv_cotizador.log'),
...
from models import (
    CotizacionInput, CotizacionResponse, EstiloSimilar, WipDisponible,
    HealthCheck, ConfiguracionResponse, WipsDisponiblesResponse, ErrorResponse
)
...
async def health_check():
    ...
    try:
        ...
        estado_general = "healthy" if all(count >= 0 for count in tablas_status.values()) else "degraded"
        estado_bd = "connected" if all(count >= 0 for count in tablas_status.values()) else "error"
...
async def verificar_estilo_completo_con_autocompletado(
    codigo_estilo: str,
    version_calculo: Optional[str] = "FLUIDA"
):
    """
    ...
    try:
        ...
        if not es_nuevo:
            try:
                info_detallada = tdv_queries.obtener_info_detallada_estilo(codigo_estilo, version_calculo)
                ...
                if info_detallada.get('encontrado', False):
                    ...
                    logger.info(f"✅ Auto-completado disponible para {codigo_estilo}: {campos_sugeridos}")
                
            except Exception as e:
                logger.warning(f"⚠️ Error obteniendo info detallada para {codigo_estilo}: {e}")
                info_detallada = None
        ...
        if not es_nuevo and info_detallada and info_detallada.get('encontrado'):
            ...
            try:
                volumen_historico = tdv_queries.obtener_volumen_historico_estilo(codigo_estilo, version_calculo)
                categoria = "Muy Recurrente" if volumen_historico >= 4000 else "Recurrente" if volumen_historico > 0 else "Nuevo"
            except:
                volumen_historico = 0
                categoria = "Nuevo"
        ...
        logger.info(f"✅ Verificación completa {codigo_estilo}: existe={existe}, auto-completado={autocompletado_disponible}")
        return respuesta
        
    except Exception as e:
        logger.error(f"❌ Error en verificación completa {codigo_estilo}: {e}")
        raise HTTPException(status_code=500, detail=f"Error verificando estilo: {str(e)}")
...
async def obtener_ops_utilizadas_cotizacion(input_data: CotizacionInput):
    ...
    """
    try:
        logger.info(f"🔍 Obteniendo OPs utilizadas para: {input_data.codigo_estilo} ({input_data.version_calculo})")
        
        # Obtener OPs utilizadas desde database
        ops_data = tdv_queries.obtener_ops_utilizadas_cotizacion(
            codigo_estilo=input_data.codigo_estilo if input_data.codigo_estilo else None,
        ...
        logger.info(f"✅ OPs encontradas: {respuesta['total_ops_encontradas']} para {input_data.codigo_estilo}")
        return respuesta
...
@app.get("/wips-disponibles", response_model=WipsDisponiblesResponse, tags=["Configuración"])
async def obtener_wips_disponibles(
    ...
    try:
        logger.info(f"🔍 Obteniendo WIPs disponibles: tipo_prenda={tipo_prenda}, version={version_calculo}")
        ...
        respuesta = WipsDisponiblesResponse(
            ...
            total_disponibles=len([w for w in wips_textiles + wips_manufactura if w.disponible]),
        ...
        logger.info(f"✅ WIPs obtenidas: {respuesta.total_disponibles} disponibles para {tipo_prenda or 'genérico'} ({version_calculo})")
        return respuesta
...
async def obtener_ruta_textil_recomendada(
    ...
    try:
        logger.info(f"🧵 Obteniendo ruta textil para: {tipo_prenda} ({version_calculo})")
        
        ruta_textil = tdv_queries.obtener_ruta_textil_recomendada(tipo_prenda, version_calculo)
        ...
        logger.info(f"✅ Ruta textil obtenida: {len(ruta_textil.get('wips_recomendadas', []))} WIPs recomendadas")
        return ruta_textil
        
    except Exception as e:
        logger.error(f"❌ Error obteniendo ruta textil para {tipo_prenda}: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo ruta textil: {str(e)}")
...
async def obtener_clientes(version_calculo: Optional[str] = "FLUIDA"):
...
async def obtener_familias_productos(version_calculo: Optional[str] = "FLUIDA"):
...
async def obtener_tipos_prenda(
    ...
    try:
        ...
        logger.info(f"✅ Tipos cargados: {len(tipos)} para {familia} ({version_calculo})")
        return respuesta
...
@app.get("/buscar-estilos/{codigo_estilo}", response_model=List[EstiloSimilar], tags=["Búsqueda"])
async def buscar_estilos_similares(
    ...
    try:
        ...
        logger.info(f"🔍 Buscando estilos similares: {codigo_estilo} para {cliente or 'cualquier cliente'} ({version_calculo})")
...
async def verificar_estilo_existente(
    ...
    try:
        ...
        if not es_nuevo:
            try:
                volumen_total = tdv_queries.obtener_volumen_historico_estilo(codigo_estilo, version_calculo)
        ...
        logger.info(f"✅ Verificación básica {codigo_estilo}: existe={existe}, categoría={categoria}, volumen={volumen_historico}")
...
async def crear_cotizacion(input_data: CotizacionInput):
    ...
    try:
        logger.info(f"💰 Nueva cotización: {input_data.usuario} | {input_data.codigo_estilo} | Versión: {input_data.version_calculo}")
        ...
        if hasattr(input_data, 'version_calculo') and input_data.version_calculo:
            ...
            if input_data.version_calculo not in versiones_validas:
                raise ValueError(f"version_calculo debe ser una de: {versiones_validas}")
        else:
            # Asignar default si no viene en input
            input_data.version_calculo = "FLUIDA"
        
        # Procesar cotización
        resultado = cotizador_tdv.procesar_cotizacion(input_data)
        
        logger.info(f"✅ Cotización completada: {resultado.id_cotizacion} | ${resultado.precio_final:.2f} | Versión: {resultado.version_calculo_usada}")
        return resultado
...
async def obtener_analisis_historico(
    ...
    try:
        ...
        logger.info(f"✅ Análisis histórico completado para {familia}/{tipo} ({version_calculo})")
        return respuesta
...
async def obtener_factor_marca(cliente: str):
    ...
    try:
        ...
        return {
            "cliente": cliente,
            "factor_marca": factor,
            "estrategia": "Aumentar Margen" if factor > 1.0 else "Mantener Volumen" if factor == 1.0 else "Reducir Margen"
        }
    except Exception as e:
        logger.error(f"Error obteniendo factor marca: {e}")
        raise HTTPException(status_code=500, detail=f"Error obteniendo factor: {str(e)}")
...
async def obtener_info_fechas_corrida(version_calculo: Optional[str] = "FLUIDA"):
    ...
    try:
        ...
        for tabla in tablas:
            try:
                fecha_max = tdv_queries.obtener_fecha_maxima_corrida(tabla, version_calculo)
                info_fechas[tabla] = {
                    "fecha_maxima_corrida": fecha_max,
                    "dias_antiguedad": (datetime.now() - fecha_max).days if fecha_max else None,
                    "estado": "actualizada" if fecha_max and (datetime.now() - fecha_max).days <= 7 else "desactualizada",
                    "version_calculo": version_calculo
                }
            except Exception as e:
                info_fechas[tabla] = {
        ...
        raise HTTPException(status_code=500, detail=f"Error obteniendo fechas: {str(e)}")
...
async def verificar_estilo_completo(
    ...
    """
    try:
        logger.info(f"🔍 Verificación completa para estilo: {codigo_estilo} (versión: {version_calculo})")
        
        # PASO 1: Verificación básica
        existe = tdv_queries.verificar_estilo_existente(codigo_estilo, version_calculo)
        es_nuevo = not existe
        
        logger.info(f"🔍 Resultado verificación básica - existe: {existe}, es_nuevo: {es_nuevo}")
        ...
        if info_detallada.get('encontrado', False):
            ...
            logger.info(f"✅ Info detallada encontrada - categoría: {categoria}, volumen: {volumen_historico}")
        else:
            categoria = "Nuevo"
            volumen_historico = 0
        ...
        if es_nuevo and familia_producto and tipo_prenda:
            try:
                ruta_recomendada = tdv_queries.obtener_ruta_textil_recomendada(tipo_prenda, version_calculo)
                wips_textiles, wips_manufactura = tdv_queries.obtener_wips_disponibles_estructurado(
                    tipo_prenda, version_calculo
                )
                ...
                resultado.update({
                    "ruta_automatica": {
                        ...
                        "wips_recomendadas": ruta_recomendada.get("wips_recomendadas", [])[:5],
                        "wips_textiles_disponibles": [
                            {"wip_id": w.wip_id, "nombre": w.nombre, "costo": w.costo_actual} 
                ...
                logger.info(f"✅ Ruta automática agregada para estilo nuevo: {codigo_estilo}")
                
            except Exception as e:
                logger.warning(f"⚠️ Error obteniendo ruta automática: {e}")
                resultado["ruta_automatica"] = {"disponible": False, "error": str(e)}
        
        logger.info(f"✅ Verificación completa finalizada para {codigo_estilo}: {resultado}")
        return resultado
...
async def autocompletar_estilo_recurrente(
    ...
    """
    try:
        logger.info(f"🔍 Auto-completado solicitado para: {codigo_estilo} (versión: {version_calculo})")
        ...
        raise HTTPException(status_code=500, detail=f"Error obteniendo autocompletado: {str(e)}")
...
async def debug_estilo_clasificacion(
    ...
    try:
        ...
        """
        resultado_historial = tdv_queries.db.execute_query(query_historial, (codigo_estilo, version_calculo))
        debug_info['historial_estilos'] = resultado_historial[0] if resultado_historial else {}
        ...
        """
        resultado_ops = tdv_queries.db.execute_query(query_ops, (codigo_estilo, version_calculo))
        ...
        """
        resultado_like = tdv_queries.db.execute_query(query_like, (f'%{codigo_estilo}%', version_calculo))
        debug_info['busqueda_similar'] = resultado_like
        
        # 4. Función actual de verificación
        existe_actual = tdv_queries.verificar_estilo_existente(codigo_estilo, version_calculo)
...
async def startup_event():
    ...
    logger.info("   - ✅ Nuevos endpoints: /verificar-estilo-completo y /ops-utilizadas-cotizacion")
    ...
    try:
        ...
        try:
            ...
            if versiones_resultado:
                ...
                logger.warning("⚠️ No se encontraron versiones de cálculo en COSTO_OP_DETALLE")
        ...
        for tabla in ['COSTO_OP_DETALLE', 'RESUMEN_WIP_POR_PRENDA']:
            try:
                for version in ['FLUIDA', 'truncado']:
                    try:
                        fecha_max = tdv_queries.obtener_fecha_maxima_corrida(tabla, version)
                        if fecha_max:
                            dias_antiguedad = (datetime.now() - fecha_max).days
                            logger.info(f"📅 {tabla} ({version}): última corrida {fecha_max} ({dias_antiguedad} días)")
                    except Exception:
