...
class CargadorOptimizado:
    ...
    def opcion_3_carga_lotes(self, cursor, sql, datos, tabla, lote_size=1000):
        ...
        tiempo = (datetime.now() - inicio).total_seconds()
        print(f"✅ {total_insertados} registros en {tiempo:.1f}s ({total_insertados/tiempo:.0f} reg/s)")
        return total_insertados

    def preparar_datos_costos_fijos(self, df):
        """Prepara datos para costos_fijos_mensuales"""
        datos = []
        for _, row in df.iterrows():
            try:
                datos.append((
                    str(row.get('TIPO_COSTO', '')).strip(),
                    str(row.get('PROCESO_PRODUCTIVO', '')).strip(),
                    int(row.get('MES', 1)),
                    int(row.get('AÑO', 2024)),
                    float(row.get('COSTO_TOTAL_USD', 0.01)),
                    datetime.now(),
                    self.lote_carga,
                    self.usuario_carga
                ))
    ...
    def preparar_datos_costos_mp(self, df):
        ...
        for _, row in df.iterrows():
            try:
                datos.append((
                    int(row.get('MES', 1)),
                    int(row.get('AÑO', 2024)),
                    float(row.get('MATERIA_PRIMA', 0.01)),
                    float(row.get('TELA_COMPRADA', 0.01)),
                    float(row.get('MATERIAL_DIRECTO', 0.01)),
                    datetime.now(),
                    self.lote_carga,
                    self.usuario_carga
                ))
    ...
    def cargar_costos_fijos(self, cursor, df, carga_inicial):
        ...
        return self.opcion_2_carga_masiva_fast_executemany(cursor, sql, datos, 'costos_fijos_mensuales')
    ...
    def cargar_costos_mp(self, cursor, df, carga_inicial):
        ...
        return self.opcion_2_carga_masiva_fast_executemany(cursor, sql, datos, 'costos_mp_ct_avios')

    def preparar_datos_bd_finanzas(self, df):
        """Prepara datos para inserción masiva en bd_finanzas"""
        datos = []
        for _, row in df.iterrows():
            if pd.isna(row.get('COD_ORDPRO')) or str(row.get('COD_ORDPRO')).strip() == '':
                continue
            ...
            datos.append((
                ...
                int(row.get('PRENDAS_REQUERIDAS', 0)) if not pd.isna(row.get('PRENDAS_REQUERIDAS')) else 0,
                row.get('FECHA_FACTURACION') if not pd.isna(row.get('FECHA_FACTURACION')) else None,
                float(monto),
    ...
    def cargar_bd_finanzas_optimizado(self, cursor, df, carga_inicial, metodo='fast'):
        ...
        if metodo == 'fast' and len(datos) > 100:
            insertados = self.opcion_2_carga_masiva_fast_executemany(cursor, sql, datos, 'bd_finanzas')
        elif metodo == 'lotes':
            insertados = self.opcion_3_carga_lotes(cursor, sql, datos, 'bd_finanzas', 1000)
        else:
    ...
    def cargar_archivo_optimizado(self, archivo_excel, carga_inicial=False, metodo='fast'):
        ...
        try:
            ...
            try:
                df_cf = pd.read_excel(archivo_excel, sheet_name='COSTOS_FIJOS_MENSUALES')
            ...
            insertados_bd = self.cargar_bd_finanzas_optimizado(cursor, df_bd, carga_inicial, metodo)
            ...
            cursor.execute("SELECT COUNT(*) FROM TDV.saya.bd_finanzas WHERE LOTE_CARGA = ?", self.lote_carga)
...
def main():
    ...
    # 1. Archivo
    archivo = input(f"📁 Excel (Enter=default): ").strip()
    if not archivo:
